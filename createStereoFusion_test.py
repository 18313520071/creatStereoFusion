...
class StereoFusionApp:
    ...
    def generate_black_white_images(self, mode='black_white'):
        """
        生成黑白融合图测试图片
        mode: 'black_white' (左黑右白) 或 'white_black' (左白右黑)
        """
        ...
        if mode == 'black_white':
            # 左黑右白
            black_img = np.zeros((h, w, 3), dtype=np.uint8)  # (0,0,0)
            white_img = np.ones((h, w, 3), dtype=np.uint8) * 255  # (255,255,255)
    ...
    def process_logic(self):
        try:
            ...
            offset_per_row_float = tan_angle * num_channels  # 浮点数偏移
            ...
            # 确保目录存在 (asksaveasfilename 通常不会自动创建目录，但为了保险)
            save_dir = os.path.dirname(final_save_path)
            if save_dir and not os.path.exists(save_dir):
                os.makedirs(save_dir)

            # 使用 imencode 保存以兼容中文路径
            ...
            self.run_btn.config(state="normal", text="开始生成融合图")
